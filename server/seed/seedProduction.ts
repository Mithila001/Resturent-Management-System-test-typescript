...
const demoUsers = [
  ...
  { name: "Owner User", email: "owner@resto.com", password: "owner123", role: "owner" },
  { name: "Manager User", email: "manager@resto.com", password: "manager123", role: "manager" },
  { name: "Chef User", email: "chef@resto.com", password: "chef123", role: "chef" },
  { name: "Waiter User", email: "waiter@resto.com", password: "waiter123", role: "waiter" },
  { name: "Cashier User", email: "cashier@resto.com", password: "cashier123", role: "cashier" },
  { name: "Customer User", email: "customer@resto.com", password: "customer123", role: "customer" },
];
...
const seedProduction = async () => {
  try {
    ...
    for (const userData of demoUsers) {
      ...
      if (existingUser) {
        console.log(
          `  ⚠️  ${userData.role.padEnd(10)} | ${userData.email.padEnd(25)} - Already exists`,
        );
        stats.users.existing++;
      } else {
        // @ts-ignore
        await User.create(userData);
        console.log(`  ✅ ${userData.role.padEnd(10)} | ${userData.email.padEnd(25)} - Created`);
        stats.users.created++;
      }
    }
    ...
    for (const item of menuItems) {
      ...
      if (category) {
        ...
        if (!existing) {
          ...
          console.log(`  ✅ ${item.name.padEnd(25)} ($${item.price}) - Created`);
          stats.menuItems.created++;
        } else {
          console.log(`  ⚠️  ${item.name.padEnd(25)} ($${item.price}) - Already exists`);
    ...
    console.log("═════════════════════════════════════════");
    console.log(`\n📊 Summary:`);
    console.log(`  Users:       ${stats.users.created} created, ${stats.users.existing} existing`);
    console.log(
    ...
    demoUsers.forEach((user) => {
      console.log(
        `  ${user.role.toUpperCase().padEnd(10)} → ${user.email.padEnd(25)} | Password: ${user.password}`,
      );
    });
    console.log("═════════════════════════════════════════\n");
